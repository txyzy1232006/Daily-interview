...
## 3.EM算法 [zhihu](https://zhuanlan.zhihu.com/p/36331115)
流程:   
...
2) $\text{for j from 1 to J}$：
  ...
  + M步：极大化 $L(\theta)$ ,得到 $\theta^{t+1}$ :
  $$\theta^{t+1} : = arg \max \limits_{\theta}\sum\limits_{i=1}^m\sum\limits_{z^{(i)}}Q_i(z^{(i)})log{P(x^{(i)}， z^{(i)}|\theta)} \\$$
  + 重复E、M步骤直到 $\theta$ 收敛  
